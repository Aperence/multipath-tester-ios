...
struct CheckMPTCPView: View {
    ...
    @State private var client: any MultipathClient = MPTCP.clients[0].client
            ...
            VStack {
                Form{
                    Section("Client selection"){
                        ClientSelectionView(client: $client, update: {
                            refresh()
                        }, mptcpOnly: true)
                    }
                    ...
                    Section("Results"){
                        if loading{
                            ...
                            ResultsView(using_mptcp: using_mptcp, transfer: MPTCPTransfers.check, response_time: response_time)
                        }
    ...
    func fetch() async {
        ...
        do{
            ...
            let data = try await client.fetch(url: MPTCPTransfers.check.url)
